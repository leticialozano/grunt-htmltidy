    ...
    grunt.registerMultiTask('htmltidy', 'Tidy HTML', function () {
        ...
        var tidyFile = function(file, index, callback) {
            ...
            if(grunt.file.isDir(file.dest)) {
                outputPath = file.dest.match(/\/$/) ? (file.dest + path.basename(src)) : file.dest + '/' + path.basename(src);
            }
            else {
            ...
            tidy(untidied, options, function(error, tidied) {
                ...
                grunt.file.write(outputPath, tidied);
                grunt.log.writeln('Tidied ' + chalk.cyan(outputPath) + ' ' + prettyBytes(untidied.length) + ' → ' + prettyBytes(tidied.length));
